...
public class MemberDAO {
	...
	//우리 개발자가 MemberDAO.java에 작성한 SQL문장을 t_member테이블에 전송하여 실행할 역할을 하는 
	//Statement 객체를 저장할 참조변수 선언
	...
	// - 이후, DriverManager.getConnection(URL, USER, PWD) 메서드를 사용하여 DB에 연결할 수 있게 됨.
			...
			//순서4. 개발자가 만든 SQL문을 미리 메모리에 올려놓고 SQL문을 오라클 XE DB의 t_member테이블에
			//전송해서 실행할 역할을 하는 Statement 객체 얻기
	...
	public ArrayList listMembers() {
		...
		try {
			...
			//요약 : DB와의 연결
			//순서4. 개발자가 작성한 SQL문장을 실행할 Statement 실행 객체 얻기
			connDB();
			...
			//순서6. SQL문장을 XE데이터베이스의 t_member테이블에 전송하여 실행(조회)
			//SQL문 조회 후 조회한 결과 데이터들을 ResultSet 객체메모리에 저장 후 반환받습니다.
			//단! 조회된 화면의 커서(화살표)위치는 가장 처음에는 조회된 테이블의 제목열 행을 가리키고 있다.
			rs = stmt.executeQuery(query); //<- ResultSet객체 반환
	...
	public void ResourceClose() {
		...
		try {
			...
			//Statement객체는 SQL 쿼리를 실행하는 객체입니다.
			//이 객체를 사용하고 있다면 제거시키자
			if( stmt != null ) {
				stmt.close();
			}			
