...
//DB작업할 삼총사 객체
	...
	*/	
	...
	//매개변수로 전닯받은 회원 ID를 이용해 수정할 회원 한사람의 정보를 조회해서
	//MemberVO객체의 각변수에 저장후 반환하는 기능의 메소드 
	public MemberVO findMember(String id) {
		...
		try {
			...
			//2.매개변수 String id로 전달받은 회원 ID열의 값에 해당하는 
			//  회원 레코드의 정보를 조회하는 SELECT문 
			...
			//4. SELECT문 실행 후 조회된 결과 데이터를 ReusltSet에 담아 반환
			rs = pstmt.executeQuery();
			
			//5. ReulstSet객체 내부에 조회된 한줄정보가 있으면?
			if(rs.next()) {
				
				//조회한 한사람(행의 정보)를 ResultSet에서 꺼내어 MemberVO객체를 생성해서 각 변수에 저장
			...
			System.out.println("findMember메소드 내부에서 SELECT문 실행 오류:" + e);
			e.printStackTrace();
		} finally {
			//자원해제
			ResourceClose();
	...
	//매개변수로 전달 받은 MemberVO객체의 정보를
	//DB의 t_member테이블의 각 컬럼(열)값을 UPDATE(수정)하는 기능의 메소드 
	public void modMember(MemberVO memberVO) {
		...
		try {
			...
			//2.매개변수로 전달받은 MemberVO객체의 id변수값에 해당되는
			...
			//3.1. ? 값 설정
			pstmt.setString(1, memberVO.getPwd()); //수정을 위해 입력한 비밀번호
		    pstmt.setString(2, memberVO.getName());//수정을 위해 입력한 이름
		    pstmt.setString(3, memberVO.getEmail());//수정을 위해 입력한 이메일
		    pstmt.setString(4, memberVO.getId()); //수정을 위한 조건 아이디값 
		    
		    //4. UPDATE문 실행
		    pstmt.executeUpdate();		
			
		} catch (Exception e) {
			System.out.println("modMember메소드 내부에서  UPDATE문 실행 오류 :" + e);
			e.printStackTrace();
		} finally {
			//5. 자원해제
			ResourceClose();
